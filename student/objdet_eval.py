...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            box = label.box
            label_bb_corners = tools.compute_box_corners(box.center_x, box.center_y, box.width, box.length, box.heading) 
            
            ## step 2 : loop over all detected objects
            for detection in detections:
                
                ## step 3 : extract the four corners of the current detection
                id_obj, x_obj, y_obj, z_obj, h_obj, w_obj, l_obj, yaw_obj = detection
                box_current_corners = tools.compute_box_corners(x_obj, y_obj, w_obj, l_obj, yaw_obj)
                
                ## step 4 : compute the center distance between label and detection bounding-box in x, y, and z
                dist_x = box.center_x - x_obj
                dist_y = box.center_y - y_obj
                dist_z = box.center_z - z_obj
                
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                polygon1 = Polygon(label_bb_corners)
                polygon2 = Polygon(box_current_corners)
