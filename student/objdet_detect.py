...
def create_model(configs):
    ...
    if (configs.arch == 'darknet') and (configs.cfgfile is not None):
        ...
        # found how to call this in class model_utils.py of the referenced algorithm SFA3D
...
def detect_objects(input_bev_maps, model, configs):
    ...
    if len(detections) > 0: 
        ...
        for detection in detections:
            ...
            # we perform computations from bev to vehicle coordinates, the opposite of what we did previously from pcl to bev
            id_v = 1   # just vehicles
            x = y_obj / configs.bev_height * (configs.lim_x[1] - configs.lim_x[0])
            y = x_obj / configs.bev_width * (configs.lim_y[1] - configs.lim_y[0]) - (configs.lim_y[1] - configs.lim_y[0]) * 0.5
            z = z_obj + configs.lim_z[0]
            l = l_obj / configs.bev_height * (configs.lim_x[1] - configs.lim_x[0])
            w = w_obj / configs.bev_width * (configs.lim_y[1] - configs.lim_y[0])
            h = h_obj
            yaw = -yaw_obj
            
            ## step 4 : append the current object to the 'objects' array
            objects.append([id_v, x, y, z, h, w, l, yaw])
