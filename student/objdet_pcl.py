...
def show_pcl(pcl):
    ...
    pcl3d = open3d.geometry.PointCloud()
    
    # step 3 : set points in pcd instance by converting the point-cloud into 3d vectors (using open3d function Vector3dVector)
    pcl3d.points = open3d.utility.Vector3dVector(pcl[:,:3])
    
    # step 4 : for the first frame, add the pcd instance to visualization using add_geometry; for all other frames, use update_geometry instead
    viz.add_geometry(pcl3d)
        
    # step 5 : visualize point cloud and keep window open until right-arrow is pressed (key-code 262)
    viz.register_key_callback(262, key_callback)
    viz.register_key_callback(65, key_callback)

    global key_press_func
    key_press_func = False

    while True:
       viz.update_geometry(pcl3d)
       viz.poll_events()
       viz.update_renderer()
       if key_press_func is not False:
          break
...
def show_range_image(frame, lidar_name):
    ...
    img_range_intensity = np.column_stack((img_range, img_intensity)).astype(np.uint8)
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    _, idx_intens_unique, counts = np.unique(lidar_pcl_cpy[:, 0:2], axis=0, return_index=True, return_counts=True)
    lidar_pcl_top = lidar_pcl_cpy[idx_intens_unique]
